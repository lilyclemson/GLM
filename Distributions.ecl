...
IMPORT ML_Core.Constants AS Core_Const;
...
EXPORT Distributions := MODULE
   ...
   */
  EXPORT REAL8 Normal_CDF(REAL8 x) := FUNCTION
    b1 := 0.319381530;
    b2 :=-0.356563782;
    b3 := 1.781477937;
    b4 :=-1.821255978;
   ...
   */
  EXPORT REAL8 Normal_PPF(REAL8 x) := FUNCTION
    p0 := -0.322232431088;
    p1 := -1.0;
    p2 := -0.342242088547;
  ...
  REAL8 Sum_Terms(INTEGER4 df, REAL8 c, REAL8 csq) := BEGINC++
    ...
    double start = (df%2==0) ? 2.0  : 3.0;
    double stop = (double) (df - 2);
    for (double ai=start; ai<=stop; ai+=2.0) {
      work_term *= csq * ((ai-1.0)/ai);
      work_sum  += work_term;
    }
    return work_sum;
  ENDC++;
   ...
   * specified value
   */
  EXPORT REAL8 T_CDF(REAL8 x, REAL8 df) := FUNCTION
   ...
   */
  EXPORT REAL8 T_PPF(REAL8 x, REAL8 df) := FUNCTION
    REAL8 PI := Core_Const.Pi; //3.14159265358979;
    REAL8 ROOT_2 := 1.414213562;
    REAL8 B21 :=  0.25;
  ...
  REAL8 Low_DF_Sum(INTEGER4 df, REAL8 x) := BEGINC++
    ...
    double start = (df%2==0) ? 2.0  : 1.0;
    double stop = (df%2==0) ? (double) (df-2)  : (double) (df-1);
    for (double ai=start; ai<=stop; ai+=2.0) {
      term *= x/ai;
      term_sum += term;
    }
   ...
   */
  EXPORT REAL8 Chi2_CDF(REAL8 x, REAL8 df) := FUNCTION
    INTEGER4 DF_Cut := 1000;
    REAL8 PI := Core_Const.Pi; //3.14159265358979;
    INTEGER4 ndf := (INTEGER4) df;
    BOOLEAN isEven := ndf%2 = 0;
  ...
  REAL8 df_lt_crit(REAL8 x, REAL8 df) := BEGINC++
    ...
    #include <math.h>
      ...
      while (iter<max_iter && curr_point < x) {
        ...
        for (double aj=1; aj<10000.0 && aj<(cut1+(cut2*term/term_sum)); aj+=1.0) {
          term = max_pos*term/(half_df+aj);
          term_sum += term;
        }
      ...
      while (iter<max_bisect && delta>min_delta && curr_point!=x) {
        ...
        for (double aj=1; aj<10000.0 && aj<(cut1+(cut2*term/term_sum)); aj+=1.0) {
          term = target_pos*term/(half_df+aj);
          term_sum += term;
        }
